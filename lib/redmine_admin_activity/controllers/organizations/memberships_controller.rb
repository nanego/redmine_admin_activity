  ...
  def journalize_updated_users(organization_functions, previous_organization_functions)
    @updated_members.each do |member|
      ...
      if limited_visibility_plugin_installed?
        ...
        next if previous_role_ids.difference(role_ids).empty? && previous_function_ids.difference(function_ids).empty?
      else
        next if previous_role_ids.difference(role_ids).empty?
      end
