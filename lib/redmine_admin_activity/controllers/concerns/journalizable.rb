module RedmineAdminActivity::Journalizable
  ...
  def add_member_journal_entry(project:, property:, prop_key:, value: nil, old_value: nil) 
  ...
  def add_member_edition_to_journal(member, previous_role_ids, role_ids, previous_function_ids = nil, function_ids = nil)
    ...
    add_member_journal_entry(project: member.project, property:'members', prop_key: prop_key, value: value, old_value: old_value)
  end

  def add_member_deletion_to_journal(member, previous_role_ids, previous_function_ids = nil)
    prop_key = limited_visibility_plugin_installed? ? 'member_roles_and_functions' : 'member_with_roles'
    add_member_journal_entry(project: member.project, property:'members', prop_key: prop_key, old_value: value_hash(member, previous_role_ids, previous_function_ids))
